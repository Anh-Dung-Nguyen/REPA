...
router.get("/impact_all_topic", async (req, res) => {
    try {
        const db = getDB();
        const impact = await db.collection("impact_all_topic")
            .find({}, { projection: { _id: 0 } })
            .toArray()
        res.json(impact);
    } catch (error) {
...
/**
 * @swagger
 * /impact/impact_all_topic/{topic_id}:
 *   get:
 *     tags:
 *       - Impact
 *     summary: Get impact details for a specific topic by topic_id
...
router.get("/impact_all_topic/:topic_id", async (req, res) => {
    const topicId = req.params.topic_id;

    try {
        const db = getDB();
        const topic = await db.collection("impact_all_topic").findOne(
            { topic_id: topicId },
            { projection: { _id: 0 } }
        );
...
/**
 * @swagger
 * /impact/impact_group_all_topic/{authorId}:
 *   get:
 *     tags:
 *       - Impact
 *     summary: Calculate group impact factor for all topics of a specific author
...
router.get("/impact_group_all_topic/:authorId", async (req, res) => {
    const { authorId } = req.params;

    try {
        const topicRes = await axios.get(`http://localhost:8000/author_topics/${authorId}`);
        const topics = topicRes.data?.[0]?.topics;

        if (!topics || topics.length === 0) {
            return res.status(404).json({ error: "No topics found for this author." });
        }

        const pythonScriptPath = path.resolve(__dirname, "../../Import_data/impact_all_topics.py");
        const pythonInterpreter = "/home/nguyen-anh-dung/mon_env/bin/python";
        const python = spawn(pythonInterpreter, [pythonScriptPath]);
