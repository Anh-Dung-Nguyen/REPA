...
/**
 * @swagger
 * /authors_papers_annotations/all_topics/author/{author_id}:
 *   get:
 *     tags:
 *       - Author with papers and annotations
 *     summary: Get list of papers with annotations and all topics (unfiltered) by authorId
 *     parameters:
 *       - in: path
 *         name: author_id
 *         required: true
 ...
 *     responses:
 *       200:
 *         description: List of papers with all topics (unfiltered) by author
 *       404:
 *         description: Not found
 *       500:
...
router.get("/all_topics/author/:author_id", async (req, res) => {
    try {
        const db = getDB();
        const authorId = req.params.author_id;
        ...
        // ✅ Fetch ALL topics (unfiltered)
        const topicsApiRes = await axios.get(
            `http://localhost:8000/author_paper_topics/author/${authorId}`
        );
        const topicsList = topicsApiRes.data;
        ...
        const enrichedPapers = await Promise.all(
            authorData.papers.map(async (paper) => {
                ...
                return {
                    ...
                    allTopics: (() => {
                        const corpusId = paper.annotation.corpusid;
                        const topics = corpusIdToTopics.get(Number(corpusId));
                        return topics || [];
                    })()
                };
            })
        ...
        console.error("Error fetching all topics for author:", err.message);
        if (err.code === 'ECONNREFUSED' || err.response?.status) {
            res.status(500).json({ error: "Error connecting to topics service" });
        } else {
            res.status(500).json({ error: "Internal server error" });
