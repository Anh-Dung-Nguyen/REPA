...
class CSOTopicImpactCalculator:
    def __init__(self, csv_file_path, specific_topics_file):
        ...
        if self.graph.nodes():
            self.max_depth = max(self.calculate_depth(t) for t in self.graph.nodes())
        if self.centrality_cache:
            for t in self.graph.nodes():
                _ = self.calculate_influence_score(t)
            self.max_influence = max(self.influence_cache.values())
    ...
    def calculate_impact_factor(self, topic_id, reference_topics):
        ...
        depth_score = depth / self.max_depth if self.max_depth > 0 else 0
        
        influence = self.calculate_influence_score(topic_id)
        influence_score = influence / self.max_influence if self.max_influence > 0 else 0
    ...
    def rank_topics_by_impact(self, topic_ids=None, top_k=10, specific_topics_only=False):
        ...
        if not reference_topics:
            sorted_topics = sorted(self.centrality_cache.items(), key=lambda x: x[1], reverse=True)
            reference_topics = [t[0] for t in sorted_topics[:len(reference_topics)]]

        results = []

        with ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(self.calculate_impact_factor, topic_id, reference_topics): topic_id
            ...
            for i, future in enumerate(as_completed(futures)):
                ...
                try:
                    impact_data = future.result()
                    if 'error' not in impact_data:
                        results.append(impact_data)
                except Exception as e:
                    print(f"Erreur sur le topic {futures[future]}: {e}", file=sys.stderr)
...
if __name__ == "__main__":
    calculator = CSOTopicImpactCalculator(
        ...
        specific_topics_file = "../Output/all_topics.txt"
    )
    ...
    df_export.to_csv("../Output/all_topics_ranked.csv", index = False, encoding = 'utf-8')
    print("Export terminé : Output/all_topics_ranked.csv")